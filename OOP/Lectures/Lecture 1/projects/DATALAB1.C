...
void PrintMenu(int x)
{
	...
	gotoxy(x,yAddNode);
	cprintf("1.Add a new Student");
	gotoxy(x,ySearchByID);
	cprintf("2.Search for a Student By his ID");
	gotoxy(x,ySearchByName);
	cprintf("3.Search for a Student By his Name");
	gotoxy(x,yDeleteById);
	cprintf("4.Delete a Student By his ID");
	gotoxy(x,yFreeList);
	cprintf("5.Delete All Students");
	gotoxy(x,yInsertByLoc);
	cprintf("6.Insert a new Student by His Location");
	gotoxy(x,yUpdateById);
	cprintf("7.Update a Student info by his ID");
	gotoxy(x,yListAllStudents);
	cprintf("8.List All Students");
	gotoxy(x,yExit);
	cprintf("9.Exit");
}
...
struct Node* SearchNode(int reqId) {
	...
	if (ptr == NULL) printf("No Student Found by this ID!");
    return ptr;
}
struct Node* SearchNodeByName(char name[]) {
    struct Node* ptr;
...
//  while (ptr -> std.name != name && ptr != NULL) {
	...
	if (ptr == NULL) printf("No Student Found by this Name!");
    return ptr;
}
void FreeList(void) {
	struct Node* ptr;
